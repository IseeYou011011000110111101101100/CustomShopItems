...
if getgenv().Rogue_AlreadyLoaded ~= nil then error("Astral was already found running or you have other scripts executed!") return else getgenv().Rogue_AlreadyLoaded = 0 end
...
local Config = {
    WindowName = "Astral | " .. game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name,
...
if makefolder and isfolder and not isfolder("Astral") then
    makefolder("Astral")

    makefolder("Astral/Configs")
...
infoSec:CreateButton("Help with a lot: WhoIsE", function()
    setclipboard("WhoIsE")
    
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "Script Notification",
        Text = "Copied WhoIsE's discord username and tag to your clipboard.",
        Duration = 5
    })
end)

infoSec:CreateButton("Join the chads.", function()
	if req then
        req({
